...
export default function AppliedJobsPage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['jobs', JobStatus.APPLIED, page, limit],
    queryFn: () => jobsClient.getJobs(JobStatus.APPLIED, page, limit),
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Applied Job Listings</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex items-center justify-center py-8">
              <p className="text-muted-foreground">No applied jobs found.</p>
            </div>
          ) : (
        ...
        {data && data.meta.totalPages > 1 && (
          <CardFooter className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Showing {((page - 1) * limit) + 1} to {Math.min(page * limit, data.meta.total)} of {data.meta.total} jobs
            </div>
